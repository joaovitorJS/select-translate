...
Objetivo: um segundo atalho global (padrão `CommandOrControl+Alt+P`), independente do principal, que mostra a tradução numa bolha pequena e sem decoração perto do cursor — sem levantar a janela principal. Desativado por padrão (opt-in, mesmo padrão do modo automático); v1 minimalista, sem botão de copiar. Ajustado a pedido do usuário depois do primeiro teste: só a tradução aparece (sem o texto original), e a bolha ajusta a altura ao tamanho do conteúdo, com um limite de 500 caracteres.
...
- [x] Testes: `#[test]` novos pra `calcular_posicao_popover` (Rust) e pra `validarConfigFormulario`/`truncarTexto` (JS) — `sincronizar_atalhos_no_backend` não é isolável em teste unitário (side effects contra o plugin de atalho global), cobertura por validação manual
- [x] Validação no Linux/WSLg (mesma técnica das correções anteriores: `xdotool`/janela Tk de teste, atalho disparado de verdade)
- [x] v2 (pedido do usuário após o primeiro teste): removido o texto original da bolha (só a tradução); `ajustarTamanhoAoConteudo()` em `popover.js` redimensiona a janela (`setSize`, permissão nova `core:window:allow-set-size`) pra caber o conteúdo — só a altura, largura fica fixa; `truncarTexto` (nova função pura em `config.js`, testada) corta a tradução em 500 caracteres antes de exibir, e `.popover-shell` ganhou um `max-height: 400px` + `overflow-y: auto` como rede de segurança

**Observações:**
- `cargo test`: 25 (19 anteriores + 6 novos de `calcular_posicao_popover`). `npm test`: 21 (19 do v1 + 2 novos de `truncarTexto`). Ambos passando.
...
- **Dois bugs reais a mais encontrados ao implementar o auto-resize (v2), só aparecendo em teste visual de verdade:**
  1. `<div class="popover-shell">` no HTML tinha só **classe**, não `id` — `document.getElementById("popover-shell")` sempre voltava `null`, e `null.scrollHeight` quebrava `ajustarTamanhoAoConteudo()` silenciosamente antes de fazer qualquer coisa (a janela nunca redimensionava, sem nenhum erro visível no terminal). Corrigido acrescentando `id="popover-shell"` no mesmo elemento.
  2. Com o bug acima corrigido, o redimensionamento ainda não crescia com um texto de teste longo — porque esse texto de teste (gerado pra simular uma tradução grande) não tinha espaços, e `white-space: pre-wrap` não quebra linha sem um ponto de quebra; o texto virava uma única linha horizontal enorme, e a altura medida (`scrollHeight`) ficava pequena. Não é um bug do popover em si (texto real de tradução sempre tem espaços), mas expôs uma lacuna real: `.bloco-texto` não tinha `overflow-wrap: break-word`, então uma palavra/URL genuinamente sem espaço (caso raro, mas possível) vazaria pra fora da caixa. Corrigido acrescentando `overflow-wrap: break-word` — e o teste manual final já foi refeito com texto realista (com espaços) pra confirmar o redimensionamento de verdade, junto com o corte em 500 caracteres (`traduz…` no fim do texto truncado).
  3. Menor, mas relevante pra quem for mexer aqui de novo: a primeira tentativa de redimensionar usava `requestAnimationFrame` pra esperar o layout — só que o evento de tradução chega **antes** do `.show()` do lado Rust (a janela ainda está escondida), e `requestAnimationFrame` fica pausado em documentos não visíveis. Trocado por leitura direta e síncrona de `scrollHeight` (que já força o recálculo do layout sozinha), sem precisar de nenhum "espera aí" assíncrono.
- Fluxo completo testado de ponta a ponta com um bypass temporário (`traducao::traduzir` substituído por um resultado fixo, só nesta sessão de teste, revertido antes de commitar) — sem isso não dava pra validar visualmente sem uma chave de API de verdade: atalho do popover captura o texto certo, bolha aparece perto do cursor só com a tradução, cresce/encolhe conforme o tamanho do texto (testado curto e longo/truncado), ganha foco de verdade.
...
- Achado à parte, do processo de teste (não do código do app): o `tauri-plugin-store` resolve `config.json` via `BaseDirectory::AppData`, que no Linux é `~/.local/share/<identifier>/`, **não** `~/.config/<identifier>/` (que é `app_config_dir()` — um diretório *diferente*, apesar do nome parecido). Editar o `config.json` manualmente no lugar errado pra testar mais rápido custou boa parte do tempo desta sessão até perceber a confusão. No Windows os dois caminhos coincidem (`%APPDATA%`), por isso nunca tinha aparecido antes.
